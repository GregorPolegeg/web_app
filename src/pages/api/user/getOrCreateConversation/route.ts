...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (!helper || !helper.member || !helper.member[0]) {
            return res.status(500).json({ message: "Failed to find a helper with member information" });
        }
        ...
        if (!conversation) {
            return res.status(500).json({ message: "Failed to find or create a conversation" });
        }
